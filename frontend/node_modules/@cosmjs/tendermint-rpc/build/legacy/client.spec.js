...
function defaultTestSuite(rpcFactory, adaptor, expected) {
    describe("create", () => {
        it("can auto-discover Tendermint version and communicate", async () => {
            testutil_spec_1.pendingWithoutTendermint();
            const client = await client_1.Client.create(rpcFactory());
        ...
        it("can connect to Tendermint with known version", async () => {
            testutil_spec_1.pendingWithoutTendermint();
            const client = await client_1.Client.create(rpcFactory(), adaptor);
...
function websocketTestSuite(rpcFactory, adaptor, expected) {
...
for (const { url, version, expected } of testutil_spec_1.tendermintInstances) {
    describe(`Client ${version}`, () => {
        it("can connect to a given url", async () => {
            testutil_spec_1.pendingWithoutTendermint();
            // default connection
            {
                const client = await client_1.Client.connect(url);
                const info = await client.abciInfo();
                expect(info).toBeTruthy();
                client.disconnect();
            }
            // http connection
            {
                const client = await client_1.Client.connect("http://" + url);
                const info = await client.abciInfo();
                expect(info).toBeTruthy();
                client.disconnect();
            }
            // ws connection
            {
                const client = await client_1.Client.connect("ws://" + url);
                const info = await client.abciInfo();
                expect(info).toBeTruthy();
                client.disconnect();
            }
        ...
        describe("With HttpClient", () => {
            const adaptor = adaptors_1.adaptorForVersion(version);
            defaultTestSuite(() => new rpcclients_1.HttpClient(url), adaptor, expected);
        });
