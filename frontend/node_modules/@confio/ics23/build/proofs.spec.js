...
describe("calculateExistenceRoot", () => {
    it("must have at least one step", () => {
        const proof = {
            key: helpers_1.toAscii("foo"),
            value: helpers_1.toAscii("bar")
        };
        expect(() => proofs_1.calculateExistenceRoot(proof)).toThrow();
    });
    it("executes one leaf step", () => {
        const proof = {
            key: helpers_1.toAscii("food"),
            value: helpers_1.toAscii("some longer text"),
            leaf: {
        ...
        expect(proofs_1.calculateExistenceRoot(proof)).toEqual(expected);
    });
    it("cannot execute inner first", () => {
        const proof = {
            ...
            path: [
                {
                    hash: codecimpl_1.ics23.HashOp.SHA256,
                    prefix: helpers_1.fromHex("deadbeef00cafe00")
                }
            ]
        };
        expect(() => proofs_1.calculateExistenceRoot(proof)).toThrow();
    ...
    it("can execute leaf then inner", () => {
        ...
        const expected = helpers_1.fromHex("836ea236a6902a665c2a004c920364f24cad52ded20b1e4f22c3179bfe25b2a9");
        expect(proofs_1.calculateExistenceRoot(proof)).toEqual(expected);
    });
});
describe("ensureSpec", () => {
    ...
    it("rejects empty proof", () => {
        ...
        expect(() => proofs_1.ensureSpec(proof, proofs_1.iavlSpec)).toThrow();
    });
    it("accepts one valid leaf", () => {
        ...
        proofs_1.ensureSpec(proof, proofs_1.iavlSpec);
    });
    ...
    it("accepts depth limited with proper number of nodes", () => {
        ...
        // fail if this throws (invalid spec)
        proofs_1.ensureSpec(proof, depthLimitedSpec);
    });
    it("rejects depth limited with too few nodes", () => {
        ...
        expect(() => proofs_1.ensureSpec(proof, depthLimitedSpec)).toThrow();
    });
