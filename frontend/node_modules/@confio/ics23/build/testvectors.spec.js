...
describe("calculateExistenceRoot", () => {
    function loadFile(filepath) {
        ...
        const commit = codecimpl_1.ics23.CommitmentProof.decode(helpers_1.fromHex(proof));
        const data = {
            root: helpers_1.fromHex(root),
            key: helpers_1.fromHex(key),
            value: value ? helpers_1.fromHex(value) : undefined
        };
    ...
    function validateTestVector(filepath, spec) {
        ...
        if (value) {
            const valid = ics23_1.verifyMembership(proof, spec, root, key, value);
            expect(valid).toBe(true);
        }
        else {
            const valid = ics23_1.verifyNonMembership(proof, spec, root, key);
            expect(valid).toBe(true);
        }
    }
    ...
    function validateBatch(proof, spec, data) {
        ...
        if (value) {
            let valid = ics23_1.verifyMembership(proof, spec, root, key, value);
            expect(valid).toBe(true);
            const items = new Map([[key, value]]);
            valid = ics23_1.batchVerifyMembership(proof, spec, root, items);
            expect(valid).toBe(true);
        }
        else {
            let valid = ics23_1.verifyNonMembership(proof, spec, root, key);
            expect(valid).toBe(true);
            const keys = [key];
            valid = ics23_1.batchVerifyNonMembership(proof, spec, root, keys);
            expect(valid).toBe(true);
        }
    }
    ...
    it("should validate compressed iavl batch exist", () => {
        ...
        const small = compress_1.compress(proof);
